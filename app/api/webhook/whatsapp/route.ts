...
const SALES_NUMBER = '919999384627';
const LOGO_IMAGE_LINK = 'https://yourdomain.com/avdmc-logo.jpg';
...
export async function POST(req: NextRequest) {
  ...
  if (field === 'user_preferences') return NextResponse.json({ success: true });
  ...
  // === 1. SILENT FORWARD EVERY OLD MESSAGE TO YOUR PERSONAL NUMBER ===
  if (text || selectedId) {
    await forwardToPersonal(from, text, selectedId, session);
  }

  if (['hi', 'hello', 'hey', 'menu', 'start'].includes(text.toLowerCase())) {
    resetSession(from);
    await sendWelcomeButtons(from);
    return NextResponse.json({ success: true });
  }
...
async function handleSelection(from: string, session: UserSession, id: string, text: string) {
  ...
  if (session.step === 'ASK_EMAIL') {
    ...
    await sendTextMessage(
      from,
      `✅ Thank you ${session.name}. Our expert will contact you shortly.`
    );

    // FINAL LEAD - Forward to your personal number
    await sendToPersonal(
      `🔥 NEW LEAD\nName: ${session.name}\nEmail: ${text}\nDest: ${session.destination}\nFrom: +${from}`
    );

    // CTA REDIRECT to your personal number
    await createCtaButton(
      from,
      'Chat directly with Jitender for best deal:',
      '💬 Chat on 9999384627',
      `Hi Jitender, Name: ${session.name}, Email: ${text}, Dest: ${session.destination}`
    );
    resetSession(from);
    return;
  }
...
// ====== FORWARD LOGIC ======
async function forwardToPersonal(
  from: string,
  text: string,
  buttonId: string,
  session: UserSession
) {
  const content = buttonId ? `Clicked: ${buttonId}` : `Msg: ${text}`;
  const msg = `👤 +${from}\n${content}\nStep: ${session.step}\nDest: ${session.destination || '-'}`;
  await sendToPersonal(msg);
}

async function sendToPersonal(body: string) {
  try {
    const res = await fetch(GRAPH_URL, {
      method: 'POST',
      headers: { Authorization: `Bearer ${TOKEN}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({
        ...
        to: SALES_NUMBER,
        type: 'text',
        text: { body: body.substring(0, 4000) },
      }),
    });
    const json = await res.json();
    console.log('FORWARD TO PERSONAL:', JSON.stringify(json));
    if (json.error) {
      console.log(
        '!!! Forward failed - You need to message your Business Number from 9999384627 once to open 24h window'
      );
    }
  } catch (e) {
    console.log(e);
...
// ====== CTA BUTTON CREATOR - REDIRECT TO 9999384627 ======
async function createCtaButton(to: string, bodyText: string, btnText: string, waMsg: string) {
  const url = `https://wa.me/${SALES_NUMBER}?text=${encodeURIComponent(waMsg)}`;
  return sendWhatsApp({
    messaging_product: 'whatsapp',
    to,
    type: 'interactive',
    interactive: {
      type: 'cta_url',
      body: { text: bodyText },
      action: { name: 'cta_url', parameters: { display_text: btnText, url } },
    },
  });
}
...
async function sendWelcomeButtons(to: string) {
  ...
  await createCtaButton(
    to,
    'Or chat directly on my personal WhatsApp:',
    '💬 Chat on 9999384627',
    'Hi Jitender, I came from AV DMC Bot'
  );
}
...
async function sendSalesContact(to: string) {
  await createCtaButton(
    ...
    '💬 Chat on +91 9999384627',
    'Hi Jitender, I need help with packages'
  );
}